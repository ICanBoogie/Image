...
class Image
{
	static public function load($file, &$info)
	{
		if (!is_file($file))
		{
			throw new \Exception(sprintf('The file <q>%s</q> does not exists', $file));
		}
		...
		if (!$info)
		{
			throw new \Exception(sprintf('Unable to get information from file <q>%s</q>', $file));
		}
		...
		switch ($mime)
		{
			...
			default:
			{
				throw new \Exception(sprintf('Unsupported image type: %s', $mime));
			}
			break;
		}
		...
		if (!$image)
		{
			throw new \Exception(sprintf('Unable to create image from %s', $file));
		}
	 ...
	 * @param string $method Resizing method, one of `RESIZE_*`.
	 * @param string|array $src Pathname of the image, or an array with the original width and
	 * height of the image to resize.
	 *
	 * @return array The final width and height of the resized image.
	 */
	static public function compute_final_size($w, $h, $method, $src)
	{
		...
		list($image_w, $image_h) = is_array($src) ? $src : getimagesize($src);
	...
	*/
	...
	static public function decode_color($color)
	{
		...
		if (isset(self::$color_names[$color]))
		{
